...
def save_simple_metrics_report(train_score: float, test_score: float, validation_score: float, model: Pipeline) -> None:
    with open('report.txt', 'w') as report_file:
        report_file.write('# Model Pipeline Descripcion\n')
 
        for key, value in model.named_steps.items():
            report_file.write(f'### {key}: {value.__repr__()}'+'\n')

        report_file.write(f'### Train Score: {str(train_score)}'+'\n')
        report_file.write(f'### Test Score: {str(test_score)}'+'\n')
        report_file.write(f'### Validation Score: {str(validation_score)}'+'\n')
